...
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.allowFiltering = colDef.allowFiltering;
    self.dataType = colDef.dataType;
    self.filterOptionsVisible = false;
    switch (self.dataType) {
        case "string":
            self.statuses = ["Contains", "Equals", "StartsWith", "EndsWith"];
            break
        case "int":
            self.statuses = ["Greater", "Lesser", "Greater Than", "Lesser Than", "Equals", "Not equals"];
            break
    }
    ...
    self.filterOptionSelected = function (status) {
        self.sample = status;
        self.filterOptionsVisible = false;
    };
    ...
    self.setVars = function (fromCol) {
        ...
        self.dataType = fromCol.dataType;
        self.allowFiltering = fromCol.allowFiltering;
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("headerCellTemplate.html",
    "<div class=\"ngHeaderSortColumn {{col.headerClass}}\" ng-style=\"{'cursor': col.cursor, 'width': col.width}\" ng-class=\"{ 'ngSorted': !noSortVisible }\"> \r" +
    "\n" +
    "    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText {{col.dataType}}\">{{col.displayName}}</div>\r" +
    "\n" +
    ...
    "</div>\r" +
    "\n" +
    "<div ng-show=\"col.showFilter()\" class=\"ngFilterContainer\" style=\"\" ng-style=\"{ 'width' : col.width + 10 + 'px' }\">\r" +
    "\n" +
    "    <input type=\"text\" ng-click=\"stopClickProp($event)\" placeholder=\"Filter...\" ng-model=\"col.filterText2\" style=\"float:left;\" ng-style=\"{ 'width' : col.width - 30 + 'px' }\" />\r" +
    "\n" +
    "    <div ng-click=\"col.showFilterOptions($event)\" class=\"ngFilter\"></div>\r" +
    "\n" +
    "    <ul class=\"ngFilterOptionList\" ng-model=\"col.sample\" ng-show=\"col.filterOptionsVisible\" style=\"z-index:9999\"><li ng-repeat=\"status in col.statuses\">{{status}}</li></ul>\r" +
    "\n" +
    "</div>\r" +
    "\n" +
  ...
  $templateCache.put("headerRowTemplate.html",
    "<div ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngHeaderCell\">\n" +
